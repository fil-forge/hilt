...
- **Tenant REST API** (`pkg/api`, echo) — partner-facing CRUD for tenants,
  ...
  permissions or buckets and holds one delegation, its marker (`pkg/marker`)
  — it names a principal (a console userId), and what that principal may do
  on a bucket is computed on each authorize from the bucket's policy; the
  per-request delegations are issued by the tenant at that moment.
...
- Integration tests: `make itest`. The `itest/` package is gated by the `itest`
  ...
  how the IAM scenarios run against ingot changes the `:main` image does not
  carry yet. Until the published `:main` ingot image carries the IAM changes,
  the IAM scenarios skip unless `HILT_ITEST_INGOT_BINARY` is set (or
  `HILT_ITEST_IAM=1`); drop that guard once it does. CI runs the suite after
  the unit job, on every push.
...
- `pkg/bucketpolicy` — the bucket policy and its evaluation: `Validate`,
  ...
  Allow), and `Changed(old, new, …)`, which names the principals whose keys a
...
- `pkg/marker` — a principal-bound key's marker: the one delegation the key
  holds (tenant → key over the tenant, command `/s3/key/marker`). It grants
  nothing and is in no proof chain; it exists so the key has a CID the gateway
  holds and Hilt can revoke. `Issue` builds it; `Rotator` revokes it through
  Swarf's `/ucan/revoke` (with a fresh nonce per revocation) and reissues it
  (`Rotate`) or leaves the key with none (`Revoke`), each key under the
  delegation store's `Replace`. `BatchTimeout` bounds a policy write's whole
  fan-out, below `store.LockTimeout`.
...
- **Locking and callbacks**: a write that another service must learn about runs
  ...
  it did not need to. Memory backends run the callback under the store mutex.
