...
- `pkg/bucketpolicy` — the bucket policy and its evaluation. The document is
  ...
  write rewrites. Pure: no store or transport dependencies.
- `pkg/grant` — an access key's stored delegations. `Issue` builds the
  tenant→key set for a list of subjects and S3 actions (one delegation per
  subject and Forge command), the shape both key kinds hold. `Rotator`
  rewrites a principal-bound key's set over one bucket as its policy changes
  (`Rotate`: revoke what the key held over the bucket, store what the new
  actions map to, a first grant revoking nothing) or empties it for a removed
  principal (`Revoke`), every key of the write in one delegation-store
  `Replace` and every revocation in one Swarf request. `BatchTimeout` bounds a
  policy write's whole rotation, below `store.LockTimeout`.
...
- **Locking and callbacks**: a write that another service must learn about runs
  ...
  locks the affected principals' rows (`principal.Store.Lock`) and rewrites
  their keys' delegations over the bucket in one delegation-store `Replace`
  (the audiences' advisory locks), publishing the revocations before anything
  is stored, so a failed publish rolls the write back and leaves the
  principal with its old access, never with more; a rollback after a
  successful publish leaves the old delegations stored and revoked at Swarf,
  so Ingot serves the key from Hilt on every request without caching until a
  committed write issues fresh delegations, and the retry republishes the
  same invocations (RFC 30, error recovery). The principal lock is what makes a key
  created for one of those principals meanwhile either part of the rotation
  or built from the committed policy. Memory backends run the callback under
  a per-store write lock; the principal store's removal lock is separate from
  its map mutex, so a removal's callback does not block reads, and its `Lock`
  holds nothing.
