...
func AuthorizeRequest(
	...
	//
	// The key signs as itself: it holds these commands over the bucket, a
	// service key from its creation and a principal-bound key from the bucket's
	// policy, which the action check above is the mirror of. If it does not,
	// the delegation simply has no proof chain to a root and is unusable —
	// harmless. The gateway obtains the chain to the credential via
	// `/s3/bucket/info`.
