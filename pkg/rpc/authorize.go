...
// itself; a principal-bound key holds no grant of its own, so the tenant signs,
// and the key's marker rides along so the gateway holds it. It returns the
// result and the delegation blocks to attach to the response. It is factored
...
func AuthorizeRequest(
	...
	// A principal-bound key's marker rides along: it is the key's one revocable
	// delegation, which the gateway holds so a policy change reaches the key
	// when Hilt revokes it. The authorizer read it before the policy. It is in
	// no proof chain, so it keys only itself.
	for _, m := range authz.Markers {
