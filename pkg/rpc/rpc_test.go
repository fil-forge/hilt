...
func TestHandlerCommands(t *testing.T) {
	...
	require.NoError(t, err)
	buckets := bucketsvc.New(zap.NewNop(), az, bucketmemory.New(), delegationmemory.New(), accesskeymemory.New(), principalmemory.New(), bucketpolicymemory.New(), up, revocations, nil)
