...
// principal nor the key is a hop in it, and the key's marker rides along for
// the gateway to hold. A bucket the principal holds no action on is reported
// as [ErrUnknownBucket].
...
// principal's effective actions on the bucket, the bucket→tenant root, and the
// key's marker. The per-request delegations the tenant signs on
// /s3/request/authorize hang off that root; the marker is the key's one
// revocable delegation, which the gateway holds so a policy change reaches the
// key when Hilt revokes it.
func (s *Service) principalInfo(ctx context.Context, b bucketstore.Record, akRec accesskey.Record) (*s3bkt.InfoOK, []ucan.Delegation, error) {
	...
	// The key's marker is read before the policy, as the authorizer reads it. A
	// policy write commits each key's new marker before it commits the policy,
	// so a reader taking the policy first could pair the old document with a
	// new marker nothing revokes. Read first, a new marker means the write
	// already holds the bucket lock, which the share-locked policy read below
	// waits on. A key holding no marker is mid-creation or mid-deletion and is
	// refused.
	page, err := s.delegations.ListByAudience(ctx, akRec.ID)
	if err != nil {
		return nil, nil, fmt.Errorf("loading the access key's marker: %w", err)
	}
	if len(page.Results) == 0 {
		return nil, nil, fmt.Errorf("%w: %s holds no marker", ErrUnknownAccessKey, akRec.ID)
	}
	...
	proofSet := map[cid.Cid][]cid.Cid{root.Link(): links}

	// The marker is in no proof chain, so it keys only itself.
	for _, m := range markers {
		proofSet[m.Link()] = []cid.Cid{m.Link()}
