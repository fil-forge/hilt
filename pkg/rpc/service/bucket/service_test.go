...
func TestCreate(t *testing.T) {
	...
	setup := func(t *testing.T, perms []string, principalBound bool, sprue bucketsvc.UploadClient, delegations delegationstore.Store, policy *did.DID) (*bucketsvc.Service, *bucketmemory.Store) {
		...
		return bucketsvc.New(zap.NewNop(), az, buckets, delegations, accessKeys, policies, sprue, &htestutil.FakeSwarf{}, nil), buckets
	}
...
func TestDelete(t *testing.T) {
	...
	setup := func(t *testing.T, perms []string, principalBound bool, sprue bucketsvc.UploadClient, grantOpts ...delegation.Option) deleteDeps {
		...
		swarf := &htestutil.FakeSwarf{}
		return deleteDeps{
			svc:         bucketsvc.New(zap.NewNop(), az, buckets, delegations, accessKeys, policies, sprue, swarf, nil),
			buckets:     buckets,
...
func TestList(t *testing.T) {
	...
	setup := func(t *testing.T, perms []string, principalBound bool) (*bucketsvc.Service, *bucketmemory.Store, did.DID) {
		...
		az := auth.NewAuthorizer(zap.NewNop(), accessKeys, tenants, providers, buckets, principals, policies, secrets)
		return bucketsvc.New(zap.NewNop(), az, buckets, delegations, accessKeys, policies, &fakeSprue{}, &htestutil.FakeSwarf{}, nil), buckets, tenantID
	}
...
func TestInfo(t *testing.T) {
	...
	setup := func(t *testing.T, perms []string, principalBound bool, grantSubject did.DID, wrap ...func(delegationstore.Store) delegationstore.Store) (*bucketsvc.Service, *bucketpolicymemory.Store, ucan.Delegation) {
		...
		return bucketsvc.New(zap.NewNop(), az, buckets, reads, accessKeys, policies, &fakeSprue{}, &htestutil.FakeSwarf{}, nil), policies, root
	}
...
func TestCreateWithPolicy(t *testing.T) {
	...
	setup := func(t *testing.T, sprue bucketsvc.UploadClient) fixture {
		...
		return fixture{bucketsvc.New(zap.NewNop(), az, buckets, delegations, accessKeys, policies, sprue, swarf, policyWrites), buckets, policies, delegations, swarf, member}
	}
	...
	t.Run("rolls back the policy with the bucket when provisioning fails", func(t *testing.T) {
		...
		// The grants the policy write issued were revoked before they went, in
		// one request, so a chain the gateway fetched meanwhile is dead too.
		require.Equal(t, 1, f.swarf.Calls())
