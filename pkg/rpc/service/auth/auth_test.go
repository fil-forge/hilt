...
import (
	...
	s3key "github.com/fil-forge/libforge/commands/s3/key"
...
func seedKey(t *testing.T, accessKeys *accesskeymemory.Store, secrets *vaultmemory.Store, delegations *delegationmemory.Store, key multikey.Issuer, tenantID did.DID, cfg *setupConfig) {
	t.Helper()
	ctx := t.Context()
	...
	if principalBound {
		// The key holds its marker, issued as the tenant.
		tenantSigner, err := ed25519.Generate()
		require.NoError(t, err)
		m, err := marker.Issue(multikey.NewIssuer(tenantID, tenantSigner), key.DID(), tenantID, expires)
		require.NoError(t, err)
...
func TestAuthorize(t *testing.T) {
	...
	// addresses.
	// dels is the delegation store of the most recent setupWithPolicies, so a
	// subtest can take the credential's marker away.
	var dels *delegationmemory.Store
	...
	setupWithPolicies := func(t *testing.T, accessKey multikey.Issuer, setupConfig *setupConfig) (*auth.Authorizer, *providermemory.Store, did.DID, did.DID, *bucketpolicymemory.Store) {
		...
		seedKey(t, accessKeys, secrets, dels, accessKey, tenantID, setupConfig)
		return auth.NewAuthorizer(zap.NewNop(), accessKeys, tenants, providers, buckets, principals, policies, dels, secrets),
	...
	t.Run("authorizes a principal-bound key for what the policy grants it", func(t *testing.T) {
		...
		require.Len(t, authz.Markers, 1, "the key's marker is carried for the container")
		require.Equal(t, s3key.Marker.Command, authz.Markers[0].Command())
	})

	t.Run("a principal-bound key holding no marker is an unknown key", func(t *testing.T) {
		az, _, tenantID, bucketID, policies := setupWithPolicies(t, accessKey, &setupConfig{principalBound: true})
		grant(t, policies, bucketID, tenantID, bucketpolicy.Statement{
			Effect:    bucketpolicy.Allow,
			Principal: bucketpolicy.Only("user-1"),
		...
		// Between a creation's row insert and its marker's store, or between a
		// deletion's marker revocation and its row delete, the key holds no
		// marker: the gateway would hold nothing to drop, so it is refused.
		require.NoError(t, dels.DeleteByAudience(ctx, accessKey.DID()))

		_, err := az.Authorize(ctx, providerID, signedObjectRequest(t, accessKey, "bucket", region))
		require.ErrorIs(t, err, auth.ErrUnknownAccessKey)
	})

	t.Run("the marker is read before the policy", func(t *testing.T) {
		// A policy write commits the new marker before the policy, so reading the
		// marker first is what keeps a request from pairing an old document with
		// a new marker.
		accessKeys, tenants := accesskeymemory.New(), tenantmemory.New()
		providers, buckets, secrets := providermemory.New(), bucketmemory.New(), vaultmemory.New()
		principals, policies, delegations := principalmemory.New(), bucketpolicymemory.New(), delegationmemory.New()
		require.NoError(t, providers.Add(ctx, providerID, region, nil))
		tenantID := testutil.RandomDID(t)
		...
		var reads []string
		az := auth.NewAuthorizer(zap.NewNop(), accessKeys, tenants, providers, buckets, principals,
			&readOrderPolicies{Store: policies, reads: &reads}, &readOrderDelegations{Store: delegations, reads: &reads}, secrets)
		_, err := az.Authorize(ctx, providerID, signedObjectRequest(t, accessKey, "bucket", region))
		require.NoError(t, err)
		require.Equal(t, []string{"marker", "policy"}, reads)
	})

	t.Run("a Deny wins over an Allow naming the same action", func(t *testing.T) {
		az, _, tenantID, bucketID, policies := setupWithPolicies(t, accessKey, &setupConfig{principalBound: true})
		grant(t, policies, bucketID, tenantID,
			bucketpolicy.Statement{Effect: bucketpolicy.Allow, Principal: bucketpolicy.Everyone(), Actions: []string{"s3:GetObject", "s3:ListBucket"}},
			bucketpolicy.Statement{Effect: bucketpolicy.Deny, Principal: bucketpolicy.Only("user-1"), Actions: []string{"s3:GetObject"}},
		)
	...
	t.Run("an empty effective set hides the bucket", func(t *testing.T) {
		...
		az, _, tenantID, bucketID, policies := setupWithPolicies(t, accessKey, &setupConfig{principalBound: true})
		grant(t, policies, bucketID, tenantID,
			bucketpolicy.Statement{Effect: bucketpolicy.Allow, Principal: bucketpolicy.Only("user-1"), Actions: []string{"s3:GetObject"}},
			bucketpolicy.Statement{Effect: bucketpolicy.Deny, Principal: bucketpolicy.Only("user-1"), Actions: []string{"s3:GetObject"}},
		)
		_, err = az.Authorize(ctx, providerID, signedObjectRequest(t, accessKey, "bucket", region))
		require.ErrorIs(t, err, auth.ErrUnknownBucket)
	...
	t.Run("a principal-bound key whose principal is gone is an unknown key", func(t *testing.T) {
		...
		gone := auth.NewAuthorizer(zap.NewNop(), accessKeys, tenants, providers, buckets,
			principalmemory.New(), bucketpolicymemory.New(), delegations, secrets)

		_, err := gone.Authorize(ctx, providerID, signedObjectRequest(t, accessKey, "bucket", region))
		require.ErrorIs(t, err, auth.ErrUnknownAccessKey)
	})

	t.Run("a principal-bound key whose principal was removed is an unknown key", func(t *testing.T) {
		// The principal exists as a tombstone: removal marks the row rather than
		// deleting it, and the authorizer must not see it.
		accessKeys, tenants := accesskeymemory.New(), tenantmemory.New()
		providers, buckets, secrets := providermemory.New(), bucketmemory.New(), vaultmemory.New()
		require.NoError(t, providers.Add(ctx, providerID, region, nil))
		tenantID := testutil.RandomDID(t)
		...
		gone := auth.NewAuthorizer(zap.NewNop(), accessKeys, tenants, providers, buckets,
			principals, bucketpolicymemory.New(), delegations, secrets)
	...
	t.Run("rejects an invalid signature", func(t *testing.T) {
		...
		az := auth.NewAuthorizer(zap.NewNop(), accessKeys, tenants, providers, bucketmemory.New(), principalmemory.New(), bucketpolicymemory.New(), delegationmemory.New(), secrets)
	...
	t.Run("rejects an unknown access key", func(t *testing.T) {
		az := auth.NewAuthorizer(zap.NewNop(), accesskeymemory.New(), tenantmemory.New(), providermemory.New(), bucketmemory.New(), principalmemory.New(), bucketpolicymemory.New(), delegationmemory.New(), vaultmemory.New())
		_, err := az.Authorize(ctx, providerID, signedRequest(t, accessKey, region, time.Now(), time.Hour))
		require.ErrorIs(t, err, auth.ErrUnknownAccessKey)
	})
...
// readOrderPolicies and readOrderDelegations append to reads when the policy
// and the marker are read, so a test can pin their order.
type readOrderPolicies struct {
	bucketpolicystore.Store
	reads *[]string
}

func (r *readOrderPolicies) Get(ctx context.Context, bucket did.DID, opts ...store.ReadOption) (bucketpolicystore.Record, error) {
	*r.reads = append(*r.reads, "policy")
	return r.Store.Get(ctx, bucket, opts...)
}

type readOrderDelegations struct {
	delegationstore.Store
	reads *[]string
}

func (r *readOrderDelegations) ListByAudience(ctx context.Context, audience did.DID, opts ...store.PaginationOption) (store.Page[ucan.Delegation], error) {
	*r.reads = append(*r.reads, "marker")
	return r.Store.ListByAudience(ctx, audience, opts...)
}
...
func TestAuthorizeLockTimeout(t *testing.T) {
	...
	_, err = policies.Put(ctx, bucketpolicystore.Input{Bucket: bucketID, Tenant: tenantID, Policy: bucketpolicy.Policy{
		Statements: []bucketpolicy.Statement{{Effect: bucketpolicy.Allow, Principal: bucketpolicy.Only("user-1"), Actions: []string{"s3:GetObject"}}},
	}}, nil)
	require.NoError(t, err)

	// With every read answered, the request is authorized.
	az := auth.NewAuthorizer(zap.NewNop(), accessKeys, tenants, providers, buckets, principals, policies, delegations, secrets)
	_, err = az.Authorize(ctx, providerID, signedObjectRequest(t, accessKey, "bucket", region))
	require.NoError(t, err)

	timeout := store.ErrLockTimeout
	cases := map[string]*auth.Authorizer{
		"access key":    auth.NewAuthorizer(zap.NewNop(), &lockedAccessKeys{accessKeys, timeout}, tenants, providers, buckets, principals, policies, delegations, secrets),
		"principal":     auth.NewAuthorizer(zap.NewNop(), accessKeys, tenants, providers, buckets, &lockedPrincipals{principals, timeout}, policies, delegations, secrets),
		"bucket policy": auth.NewAuthorizer(zap.NewNop(), accessKeys, tenants, providers, buckets, principals, &lockedPolicies{policies, timeout}, delegations, secrets),
	}
...
func TestTenantIssuer(t *testing.T) {
	...
	az := auth.NewAuthorizer(zap.NewNop(), accesskeymemory.New(), tenantmemory.New(), providermemory.New(), buckets, principalmemory.New(), bucketpolicymemory.New(), delegationmemory.New(), secrets)
