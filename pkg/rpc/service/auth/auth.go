...
type AuthorizedRequest struct {
	...
	Principal *string
	// Markers are the marker delegations a principal-bound key holds (one, in
	// the steady state), read before the policy so the gateway never pairs a
	// new marker with an old document. Nil for a service credential.
	Markers []ucan.Delegation
...
// principal-bound key's marker is read before the policy, and a key holding
// none is refused ([ErrUnknownAccessKey]).
//
...
func (a *Authorizer) Authorize(ctx context.Context, issuer did.DID, req s3.Request) (*AuthorizedRequest, error) {
	...
	// A principal-bound key's marker is read before the policy. A policy write
	// commits each key's new marker before it commits the policy, so a request
	// that read the old policy before the write took the bucket lock and the
	// new marker after it would cache the old permissions under a marker
	// nothing revokes. Read first, a new marker means the write already holds
	// the lock, and the share-locked policy read below waits for its commit. A
	// key holding no marker is mid-creation or mid-deletion and is refused: the
	// gateway would hold nothing to drop.
	var markers []ucan.Delegation
	if akRec.Principal != nil {
		page, err := a.delegations.ListByAudience(ctx, accessKeyID)
		if err != nil {
			log.Error("looking up access key marker", zap.Error(err))
			return nil, fmt.Errorf("looking up access key marker: %w", err)
		}
		if len(page.Results) == 0 {
			log.Debug("rejecting principal-bound access key holding no marker")
			return nil, ErrUnknownAccessKey
		}
