...
func CreateBucket(
	...
	if authz.Operation != auth.OpCreateBucket {
		return nil, nil, fmt.Errorf("request is not a CreateBucket operation: %s", authz.Operation)
	}

	_, err = buckets.GetByName(ctx, authz.BucketName)
	if err == nil {
		return nil, nil, fmt.Errorf("bucket %q already exists", authz.BucketName)
	...
	log := logger.With(zap.Stringer("bucket", bucketID), zap.String("name", authz.BucketName))

	if err := buckets.Add(ctx, bucketID, authz.Tenant.ID, authz.BucketName); err != nil {
	...
	subscription, err := provisioner.ProvisionSpace(ctx, account, bucketID)
	if err != nil {
		rollback()
	...
	for _, d := range stored {
		...
		proofs, links, err := delegations.ProofChain(ctx, accessKeyID, d.Command(), bucketID)
		if err != nil {
