...
func TestAuthorizeRequest(t *testing.T) {
	...
	// principal of the tenant and the key holds only its marker.
	setup := func(t *testing.T, perms []string, principalBound bool, vaultSigner ed25519.Signer) (*auth.Authorizer, *bucketpolicymemory.Store) {
		...
		if principalBound {
			...
			require.NoError(t, err)
			require.NoError(t, dels.PutBatch(ctx, []ucan.Delegation{m}))
		}
		...
		return auth.NewAuthorizer(zap.NewNop(), accessKeys, tenants, providers, buckets, principals, policies, dels, secrets), policies
	}
	...
	t.Run("a principal-bound key gets tenant-signed delegations, its marker and its effective set", func(t *testing.T) {
		...
		// The tenant issues the per-request delegation: the principal holds no
		// grant of its own and the key is not a hop in the chain. The key's marker
		// rides along, keyed to itself, for the gateway to hold.
		require.Len(t, blocks, 2)
		var reDel, m ucan.Delegation
		for _, d := range blocks {
		...
		require.Equal(t, []cid.Cid{m.Link()}, ok.Delegations.Entries[m.Link()])
		require.Equal(t, []cid.Cid{reDel.Link()}, ok.Delegations.Entries[reDel.Link()])
	})

	t.Run("a service key keeps signing its own delegations and carries no marker", func(t *testing.T) {
		az, _ := setup(t, []string{"s3:GetObject"}, false, akSigner)
		args := signedGetArgs(t, akSigner, bucketName, region, time.Now(), time.Hour)

		_, blocks, err := call(t, az, providerID, args)
		require.NoError(t, err)
		require.Len(t, blocks, 1)
	...
	t.Run("a bucket-configuration read issues no delegation beyond the marker", func(t *testing.T) {
		// s3:GetBucketVersioning maps to no Forge command: Ingot answers it from
		// its registry, so the gateway needs the permission and, for a
		// principal-bound key, the marker it holds for every request.
		az, policies := setup(t, nil, true, akSigner)
		...
		require.NoError(t, err)
		require.Len(t, blocks, 1)
		require.Equal(t, marker.Command, blocks[0].Command())
		require.Equal(t, map[cid.Cid][]cid.Cid{blocks[0].Link(): {blocks[0].Link()}}, ok.Delegations.Entries)
