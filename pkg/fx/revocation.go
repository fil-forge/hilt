...
// principal invalidation publisher, which signs as Hilt rather than as a tenant.
var RevocationModule = fx.Module("revocation",
	fx.Provide(
		fx.Annotate(
			...
			fx.As(new(invalidation.Invalidator)),
		),
		fx.Annotate(
			invalidation.NewSwarfPublisher,
			fx.As(new(invalidation.Publisher)),
		),
	),
)
