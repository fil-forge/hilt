...
func TestManagementClient(t *testing.T) {
	...
	t.Run("ProvisionTenant returns the created tenant (201)", func(t *testing.T) {
		c := newClient(t, func(w http.ResponseWriter, r *http.Request) {
			...
			require.Equal(t, "Acme", body.DisplayName)
			require.Equal(t, "us-east-1", body.Region)
			w.WriteHeader(http.StatusCreated)
			_ = json.NewEncoder(w).Encode(api.Tenant{TenantID: "acme", DisplayName: "Acme", Status: api.TenantStatusActive})
		})
		got, err := c.ProvisionTenant(ctx, "acme", api.ProvisionTenantRequest{DisplayName: "Acme", Region: "us-east-1"})
		require.NoError(t, err)
