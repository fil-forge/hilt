...
func (c *Client) putBucketPolicy(ctx context.Context, tenantID, bucketName string, doc api.BucketPolicy, headers map[string]string, wantStatus int) (string, error) {
	...
	return etagOf(header)
}

// etagOf returns the response's ETag, or [ErrMissingETag] when it carries none.
func etagOf(header http.Header) (string, error) {
...
// doWithHeaders executes a single request: it builds the URL with
// [Client.resolve], sets auth/JSON headers plus any extra ones, sends
