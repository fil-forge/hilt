...
-- is authorized from the bucket policies, so both columns are NULL for it.
-- Existing rows have no principal and need no backfill.
ALTER TABLE access_key
    ALTER COLUMN permissions DROP NOT NULL,
    ADD COLUMN principal_id TEXT,                         -- console principalId
    ADD CONSTRAINT access_key_principal_fkey
        FOREIGN KEY (tenant_id, principal_id) REFERENCES principal(tenant_id, external_id) ON DELETE RESTRICT,
...
-- Principal-bound keys hold NULL permissions; the pre-IAM column is NOT NULL.
UPDATE access_key SET permissions = '{}' WHERE permissions IS NULL;
ALTER TABLE access_key ALTER COLUMN permissions SET NOT NULL;
-- +goose StatementEnd
