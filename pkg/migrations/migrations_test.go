...
func TestAccessKeyIAMDown(t *testing.T) {
	t.Run("refuses a tenant holding two keys of one name", func(t *testing.T) {
		...
		exec(t, ctx, db, `INSERT INTO access_key (id, tenant_id, name, principal_id)
	...
	t.Run("rolls back when no tenant holds two keys of one name", func(t *testing.T) {
		...
		// The pre-IAM schema is back: no principal_id column, permissions NOT NULL
		// again (the two principal-bound rows were given empty sets), and the
		// tenant-wide unique constraint again refuses a second key of the same
		// name.
		var nullPerms int
		require.NoError(t, db.QueryRowContext(ctx, `SELECT COUNT(*) FROM access_key WHERE permissions IS NULL`).Scan(&nullPerms))
		require.Equal(t, 0, nullPerms)
		var exists bool
		require.NoError(t, db.QueryRowContext(ctx, `SELECT EXISTS (
			SELECT 1 FROM information_schema.columns
			WHERE table_name = 'access_key' AND column_name = 'principal_id')`).Scan(&exists))
