...
func (s *Service) Create(ctx context.Context, externalID, name string, permissions, bucketNames []string, principalID string, expiresAt *time.Time) (accesskeystore.Record, string, error) {
	...
	if principalID != "" {
		...
		_, err := s.principals.Get(ctx, tenantRec.ID, principalID, store.WithLock(store.LockShare))
		if errors.Is(err, store.ErrRecordNotFound) {
	...
	if principalRef != nil {
		policies, err := s.policies.ListByPrincipal(ctx, tenantRec.ID, principalID, store.WithLock(store.LockShare))
		if err != nil {
			rollback()
