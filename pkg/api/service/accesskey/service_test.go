...
func TestCreatePrincipalBound(t *testing.T) {
	...
	t.Run("binds the key to the principal and issues its marker", func(t *testing.T) {
		d := setup(t)
		...
		// The key's one delegation is its marker: tenant → key over the tenant,
		// living as long as the key does.
		issued, err := d.delegations.ListByAudience(ctx, rec.ID)
		require.NoError(t, err)
		require.Len(t, issued.Results, 1, "a principal-bound key holds only its marker")
		m := issued.Results[0]
		require.Equal(t, d.tenantID, m.Issuer())
		require.Equal(t, rec.ID, m.Audience())
		require.Equal(t, d.tenantID, m.Subject())
	...
	t.Run("persists an expiry", func(t *testing.T) {
		...
		// The marker expires with the key.
		issued, err := d.delegations.ListByAudience(ctx, rec.ID)
		require.NoError(t, err)
		require.Len(t, issued.Results, 1)
		require.NotNil(t, issued.Results[0].Expiration())
		require.Equal(t, ucan.UnixTimestamp(exp.Unix()), *issued.Results[0].Expiration())
	})
