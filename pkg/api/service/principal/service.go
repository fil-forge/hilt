...
// delegation, and appears in no UCAN; each key bound to it holds only its
// marker delegation.
//
// Removal is the one operation that spans three tables. It runs under the
// principal store's row lock: the callback revokes the markers of the
...
func (s *Service) Delete(ctx context.Context, externalID, principalID string) error {
	...
	if err := s.principals.Delete(ctx, tenantID, principalID, func(ctx context.Context) error {
		...
		// deadline ([marker.BatchTimeout]) is below the stores' lock timeout,
...
// the markers of the principal's keys, strip the principal from every policy
...
// [marker.BatchTimeout], below the wait a share-locked reader gives the row.
func (s *Service) remove(ctx context.Context, tenantID did.DID, principalID string) error {
	...
	defer cancel()
	if err := s.markers.Revoke(ctx, tenantID, principalID); err != nil {
		return err
	}
...
func (s *Service) stripFromPolicies(ctx context.Context, tenantID did.DID, principalID string) error {
	...
	for _, rec := range recs {
		doc, changed := withoutPrincipal(rec.Policy, principalID)
		if !changed {
			continue
		}
		etag := rec.ETag
...
func (s *Service) deleteKeys(ctx context.Context, tenantID did.DID, principalID string) error {
	...
	for _, rec := range recs {
		// The key's marker is already revoked and gone, so the deletion publishes
		// nothing of its own.
...
// withoutPrincipal returns doc with principalID removed from every statement
...
func withoutPrincipal(doc bucketpolicy.Policy, principalID string) (bucketpolicy.Policy, bool) {
	...
	for _, st := range doc.Statements {
		...
		statements = append(statements, bucketpolicy.Statement{
			Sid:       st.Sid,
			Effect:    st.Effect,
			Principal: bucketpolicy.Only(ids...),
			Actions:   st.Actions,
		})
	}
