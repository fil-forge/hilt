...
// fakeSwarf is a stub of the revocation service, recording the delegations it
// was asked to revoke.
...
func (f *fakeSwarf) Publish(_ context.Context, _ ucan.Issuer, revoked ucan.Delegation, _ ...swarfclient.PublishOption) error {
	...
	f.revoked = append(f.revoked, revoked.Link())
	return nil
}
...
func setup(t *testing.T) deps {
	...
	// tenant-1's key is in the vault: the marker rotator signs revocations as
	// the tenant.
	...
	d.markers = marker.NewRotator(zap.NewNop(), d.delegations, d.accessKeys, d.secrets, d.swarf)
	d.svc = principalsvc.New(zap.NewNop(), d.tenants, d.principals, d.policies, d.accessKeys, d.secrets, d.markers)
	return d
}
...
// tenant-1, its marker the way the access-key service's create route does, so
// the removal path has one to find.
func (d deps) addKey(t *testing.T, tenantID did.DID, userID, name string) did.DID {
	...
	if tenantID == d.tenantID {
		m, err := marker.Issue(d.tenant, id, tenantID, nil)
		require.NoError(t, err)
...
func TestDelete(t *testing.T) {
	...
	t.Run("revokes its keys' markers, strips the principal from policies, and deletes its keys", func(t *testing.T) {
		d := setup(t)
		for _, id := range []string{"user-1", "user-2"} {
			_, _, err := d.svc.Create(ctx, "tenant-1", id)
			require.NoError(t, err)
		...
		// Its keys, their markers and their vault entries are gone; another
		// principal's are not.
	...
	t.Run("a publish failure leaves the principal, its keys and its policies intact", func(t *testing.T) {
		...
		require.Len(t, d.held(t, key), 1, "its marker must survive")
		_, err = d.secrets.Read(ctx, vault.AccessKeyPath(d.tenantID, key))
...
func TestDeleteConcurrentChange(t *testing.T) {
	...
	t.Run("a policy edited during the removal is a retryable conflict", func(t *testing.T) {
		...
		svc := principalsvc.New(zap.NewNop(), d.tenants, d.principals, policies, d.accessKeys, d.secrets, d.markers)
	...
	t.Run("a lock the store gave up on is a retryable conflict", func(t *testing.T) {
		...
		svc := principalsvc.New(zap.NewNop(), d.tenants, principals, d.policies, d.accessKeys, d.secrets, d.markers)
		require.ErrorIs(t, svc.Delete(ctx, "tenant-1", "user-1"), principalsvc.ErrConcurrentChange)
	})
}
