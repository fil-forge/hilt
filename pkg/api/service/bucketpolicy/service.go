...
//
// Every write rotates the markers of the keys of each principal whose
...
// only when every rotation succeeded, so a principal never keeps access the
...
//
// The markers of the principals the change affects are rotated inside the
...
func (s *Service) Put(ctx context.Context, externalID, bucketName string, doc bucketpolicy.Policy, ifMatch *string) (string, bool, error) {
	...
	etag, err := s.policies.Put(ctx, bucketpolicystore.Input{
		...
		// same lock, so rotating its markers here reaches it in time.
		principals, err := s.tenantPrincipals(ctx, tenantID)
		if err != nil {
			return err
		}
		return s.rotate(ctx, tenantID, bucketpolicy.Changed(oldDoc, &doc, principals))
	})
	if err != nil {
...
// Every principal the policy reached loses its access, so each one's markers
// are rotated inside the store's transaction.
func (s *Service) Delete(ctx context.Context, externalID, bucketName, ifMatch string) error {
	...
	err = s.policies.Delete(ctx, b.ID, ifMatch, func(ctx context.Context, old bucketpolicystore.Record) error {
		...
		return s.rotate(ctx, tenantID, bucketpolicy.Changed(&old.Policy, nil, principals))
	})
	if errors.Is(err, store.ErrRecordNotFound) {
...
// publishConcurrency is how many principals' rotations of one write are in
// flight at once. The revocation service takes them in any order.
const publishConcurrency = 4

// rotate rotates the markers of each principal's keys, a few principals at a
// time, under one deadline for the whole batch: it runs inside the store's
// write transaction while the bucket is locked, so the batch is bounded at
// [marker.BatchTimeout] rather than each publish in turn (see there). Any
// failure, the deadline included, fails the write and rolls it back.
func (s *Service) rotate(ctx context.Context, tenantID did.DID, principals []string) error {
	ctx, cancel := context.WithTimeout(ctx, marker.BatchTimeout)
	defer cancel()
	g, ctx := errgroup.WithContext(ctx)
	g.SetLimit(publishConcurrency)
	for _, p := range principals {
		g.Go(func() error {
			if err := s.markers.Rotate(ctx, tenantID, []string{p}); err != nil {
				return fmt.Errorf("rotating the markers of principal %q: %w", p, err)
			}
			return nil
		})
	}
...
// timeout it runs under, and the write is retried the same way. A document
// the store refuses for naming a principal the tenant no longer has (removed
// between the validation above and the write) is an invalid policy.
func (s *Service) writeError(bucketName string, err error) error {
	switch {
	case errors.Is(err, store.ErrPreconditionFailed):
		return ErrPreconditionFailed
	case errors.Is(err, store.ErrLockTimeout), errors.Is(err, context.DeadlineExceeded):
