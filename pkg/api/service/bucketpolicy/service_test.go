...
// removal revokes its keys' markers first, so the signal says the removal is
// inside the principal store's write.
...
func (s *signalingSwarf) Publish(ctx context.Context, issuer ucan.Issuer, revoked ucan.Delegation, opts ...swarfclient.PublishOption) error {
	s.once.Do(func() { close(s.started) })
	return s.fakeSwarf.Publish(ctx, issuer, revoked, opts...)
}
...
func TestRotations(t *testing.T) {
	...
	t.Run("a create rotates the markers of the principals it grants", func(t *testing.T) {
		...
		for key, principal := range d.keys {
			...
			require.Equal(t, s3key.Marker.Command, held[0].Command())
			if principal == "user-3" {
	...
	t.Run("a batch of publishes that never answer fails the write at the batch deadline", func(t *testing.T) {
		...
		start := time.Now()
		_, _, err = svc.Put(ctx, "tenant-1", "photos", doc(allow(everyone, "s3:GetObject")), &etag)
