...
func (s *Service) Provision(ctx context.Context, externalID, region string) (tenantstore.Record, bool, error) {
	...
	// the vault.
	if err := s.wrapKeys.Add(ctx, wrapkeystore.Input{
		Tenant:   tenantID,
		Version:  1,
...
func (s *Service) Delete(ctx context.Context, externalID string) error {
	...
	wrapKeyRecs, err := s.wrapKeys.List(ctx, rec.ID)
	if err != nil {
		log.Error("listing wrap keys", zap.Error(err))
		return fmt.Errorf("listing wrap keys: %w", err)
	}
	for _, wk := range wrapKeyRecs {
		if err := s.secrets.Delete(ctx, wk.VaultKey); err != nil {
			log.Warn("removing wrap key from vault", zap.Error(err))
		}
	}
	if err := s.wrapKeys.Delete(ctx, rec.ID); err != nil {
	...
	if err := s.secrets.Delete(ctx, signingVaultKey); err != nil {
...
// revocation for every delegation the keys hold. Nothing is published when
// they hold none.
func (s *Service) revokeKeyDelegations(ctx context.Context, log *zap.Logger, tenantID did.DID, keys []accesskey.Record) error {
	...
	for _, ak := range keys {
		held, err := store.Collect(ctx, func(ctx context.Context, opts store.PaginationConfig) (store.Page[ucan.Delegation], error) {
			var listOpts []store.PaginationOption
			if opts.Cursor != nil {
				listOpts = append(listOpts, store.WithCursor(*opts.Cursor))
			}
			return s.delegations.ListByAudience(ctx, ak.ID, listOpts...)
		})
		if err != nil {
			return fmt.Errorf("listing access key delegations: %w", err)
		}
		dels = append(dels, held...)
	}
	if len(dels) == 0 {
		return nil
	}
	...
	if err := grant.PublishRevocations(ctx, log, s.revocations, issuer, dels); err != nil {
		return fmt.Errorf("revoking the tenant's key delegations: %w", err)
	}
	return nil
}
