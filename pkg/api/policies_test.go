...
func setupPolicies(t *testing.T) (*echo.Echo, *policyDeps) {
	...
	// No principal holds a key here, so the marker rotator has nothing to
	// publish; the routes are what is under test.
	markers := marker.NewRotator(zap.NewNop(), delegationmemory.New(), accesskeymemory.New(), vaultmemory.New(), noopRevocations{})
	svc := bucketpolicysvc.New(zap.NewNop(), tenants, deps.buckets, deps.principals, bucketpolicymemory.New(), markers)
	e := echo.New()
