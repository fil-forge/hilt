...
func setupPolicies(t *testing.T) (*echo.Echo, *policyDeps) {
	...
	grants := grant.NewRotator(zap.NewNop(), delegationmemory.New(), accesskeymemory.New(), vaultmemory.New(), noopRevocations{})
...
func TestBucketPolicyHandlers(t *testing.T) {
	...
	t.Run("deletes with If-Match and answers 404 once the policy is gone", func(t *testing.T) {
		...
		require.Equal(t, api.Error{Code: "PolicyNotFound", Message: "bucket has no policy"}, decodeError(t, rec))
	})
	...
	t.Run("a document the caller may not store is 422", func(t *testing.T) {
		...
		for _, body := range [][]byte{
			policyBody(t, bucketpolicy.Statement{Effect: bucketpolicy.Allow, Principal: bucketpolicy.Only("ghost"), Actions: []string{"s3:GetObject"}}),
			policyBody(t, allowUser1("s3:CreateBucket")),
			policyBody(t),
			// Strict decoding: a field outside the schema, the wildcard inside a
			// list, and the capitalized effect of AWS documents are all refused.
			[]byte(`{"statement":[{"effect":"allow","principal":["user-1"],"action":["s3:GetObject"],"resource":"photos"}]}`),
			[]byte(`{"statements":[{"effect":"allow","principals":["user-1"],"actions":["s3:GetObject"]}]}`),
			[]byte(`{"statement":[{"effect":"allow","principal":["user-1","*"],"action":["s3:GetObject"]}]}`),
			[]byte(`{"statement":[{"effect":"Allow","principal":["user-1"],"action":["s3:GetObject"]}]}`),
		} {
	...
	t.Run("stores the wildcard principal, a sid and the action wildcard as written", func(t *testing.T) {
		e, _ := setupPolicies(t)
		body := []byte(`{"statement":[` +
			`{"sid":"everyone","effect":"allow","principal":"*","action":["s3:GetObject"]},` +
			`{"effect":"allow","principal":["user-1"],"action":["s3:*"]}]}`)
		createPolicy(t, e, body)
		...
		require.Equal(t, bucketpolicy.Policy{Statements: []bucketpolicy.Statement{
			{Sid: "everyone", Effect: bucketpolicy.Allow, Principal: bucketpolicy.Everyone(), Actions: []string{"s3:GetObject"}},
			{Effect: bucketpolicy.Allow, Principal: bucketpolicy.Only("user-1"), Actions: []string{"s3:*"}},
		}}, got)
		require.JSONEq(t, string(body), rec.Body.String(), "the canonical form keeps the document as written")
	})

	t.Run("a missing bucket, a foreign bucket and a bucket without a policy are 404", func(t *testing.T) {
		e, _ := setupPolicies(t)

		// All three are 404; the code is what separates them. A bucket that is
		// missing or another tenant's is one the caller may not see, and a
		// bucket with no policy is one it may write the first statement on.
		var bodies []api.Error
		for _, path := range []string{
			"/tenants/tenant-1/buckets/nope/policy",
			"/tenants/tenant-2/buckets/photos/policy",
			policyPath,
		} {
			rec := doRequest(t, e, http.MethodGet, path, nil)
			require.Equal(t, http.StatusNotFound, rec.Code)
			bodies = append(bodies, decodeError(t, rec))
		}

		require.Equal(t, []api.Error{
			{Code: "BucketNotFound", Message: "bucket not found"},
			{Code: "BucketNotFound", Message: "bucket not found"},
			{Code: "PolicyNotFound", Message: "bucket has no policy"},
		}, bodies)
	})
