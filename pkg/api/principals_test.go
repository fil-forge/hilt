...
func setupPrincipals(t *testing.T) (*echo.Echo, *principalDeps) {
	...
	// The tenant's key is in the vault: the marker rotator signs revocations as
	// the tenant.
	...
	markers := marker.NewRotator(zap.NewNop(), deps.delegations, deps.accessKeys, deps.vault, deps.revocations)
	svc := principalsvc.New(zap.NewNop(), tenants, deps.principals, deps.policies, deps.accessKeys, deps.vault, markers)
	e := echo.New()
...
func addPrincipalKey(t *testing.T, deps *principalDeps, userID, name string) did.DID {
	...
	m, err := marker.Issue(deps.tenant, id, deps.tenantID, nil)
	require.NoError(t, err)
	require.NoError(t, deps.delegations.PutBatch(ctx, []ucan.Delegation{m}))
	return id
}
...
func TestDeletePrincipalHandler(t *testing.T) {
	...
	t.Run("removes the principal, its keys and its access, and is idempotent", func(t *testing.T) {
		...
		require.Len(t, deps.revocations.revoked, 1, "the key's marker is revoked")
	...
	t.Run("a failed revocation is 500 and changes nothing", func(t *testing.T) {
		...
		dels, err := deps.delegations.ListByAudience(ctx, keyID)
		require.NoError(t, err)
		require.Len(t, dels.Results, 1, "its marker must survive")
	})
