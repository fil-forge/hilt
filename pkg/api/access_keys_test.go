...
func (r *recordingRevocations) PublishBatch(_ context.Context, _ ucan.Issuer, revoked []ucan.Delegation) error {
	if r.err != nil {
		return r.err
	}
	for _, d := range revoked {
		r.revoked = append(r.revoked, d.Link())
	}
	return nil
}
...
func TestCreateAccessKeyHandler(t *testing.T) {
	...
	t.Run("creates a bucket-scoped key and issues delegations", func(t *testing.T) {
		...
		// 7 delegations: /content/retrieve + /blob/add + /index/add + /upload/add +
		// /upload/remove + /blob/abort + /blob/remove, all scoped to the bucket,
		// issued by the tenant to the access key.
		dels, err := deps.delegations.ListByAudience(ctx, akID)
		require.NoError(t, err)
		require.Len(t, dels.Results, 7)
...
func TestCreatePrincipalBoundAccessKeyHandler(t *testing.T) {
	...
	t.Run("creates a key bound to the principal with no authority of its own", func(t *testing.T) {
		...
		// The key pair is in the vault under the access-key path. No policy names
		// alice, so the key holds no delegation.
		_, err = deps.vault.Read(ctx, "/tenant/"+deps.tenantID.String()+"/access-key/"+akID.String())
		require.NoError(t, err)
		dels, err := deps.delegations.ListByAudience(ctx, akID)
		require.NoError(t, err)
...
func TestDeleteAccessKeyHandler(t *testing.T) {
	...
	t.Run("a lock the store gave up on is 409", func(t *testing.T) {
		...
		svc := accesskeysvc.New(zap.NewNop(), deps.tenants, locked, deps.principals, deps.buckets, bucketpolicymemory.New(), deps.delegations, deps.vault, noopRevocations{})
		lockedEcho := echo.New()
