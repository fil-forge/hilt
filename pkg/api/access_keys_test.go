...
func TestCreatePrincipalBoundAccessKeyHandler(t *testing.T) {
	...
	t.Run("creates a key bound to the principal holding only its marker", func(t *testing.T) {
		e, deps := setupAccessKeys(t)
		...
		// The key pair is in the vault under the access-key path, and the key's
		// one delegation is its marker.
		_, err = deps.vault.Read(ctx, "/tenant/"+deps.tenantID.String()+"/access-key/"+akID.String())
		require.NoError(t, err)
		dels, err := deps.delegations.ListByAudience(ctx, akID)
		require.NoError(t, err)
		require.Len(t, dels.Results, 1)
		require.Equal(t, key.Marker.Command, dels.Results[0].Command())
		require.Equal(t, akID, dels.Results[0].Audience())
	})
