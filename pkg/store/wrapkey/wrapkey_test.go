...
// dummyKID builds a unique, stable stand-in kid for a (tenant, version) in
// tests. Real kids are the multibase multicodec-tagged X25519 public key; the
// store only cares that it is a unique string.
func dummyKID(tenant did.DID, version int) string {
...
func TestWrapKeyStore(t *testing.T) {
	for _, k := range storeKinds {
		t.Run(string(k), func(t *testing.T) {
			...
			t.Run("GetByKID returns ErrRecordNotFound for unknown kid", func(t *testing.T) {
				_, err := s.GetByKID(t.Context(), "z6LSnope")
				require.ErrorIs(t, err, store.ErrRecordNotFound)
			})
			...
			t.Run("Add rejects a second active key for a tenant", func(t *testing.T) {
				...
				second := rec
				second.Version = 2
				second.KID = dummyKID(rec.Tenant, 2)
