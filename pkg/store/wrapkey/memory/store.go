...
func (s *Store) Add(ctx context.Context, rec wrapkey.Record) error {
	...
	for _, existing := range s.keys {
		// kid is globally unique (it is the key's fingerprint).
		if existing.KID == rec.KID {
			return store.ErrRecordExists
		}
		// At most one active key per tenant.
		if rec.Status == wrapkey.Active && existing.Tenant == rec.Tenant && existing.Status == wrapkey.Active {
			return store.ErrRecordExists
		}
	}
