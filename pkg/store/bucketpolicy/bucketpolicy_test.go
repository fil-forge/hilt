		...
		t.Run(string(k), func(t *testing.T) {
			...
			t.Run("Put creating passes a nil old record to the callback", func(t *testing.T) {
				...
				_, err := s.Put(t.Context(), bucketpolicystore.Input{
					Bucket: bucketID, Tenant: tenantID, Policy: doc(allow([]string{"alice"}, "s3:GetObject")),
			...
			t.Run("Put with nil IfMatch fails when a policy exists and writes nothing", func(t *testing.T) {
				...
				called := false
				_, err = s.Put(t.Context(), bucketpolicystore.Input{
					Bucket: bucketID, Tenant: tenantID, Policy: doc(allow([]string{"bob"}, "s3:GetObject")),
			...
			t.Run("Put with a stale IfMatch fails and writes nothing", func(t *testing.T) {
				...
				called := false
				_, err = s.Put(t.Context(), bucketpolicystore.Input{
					Bucket: bucketID, Tenant: tenantID, Policy: doc(allow([]string{"bob"}, "s3:GetObject")), IfMatch: ptr(etag1),
				}, func(context.Context, *bucketpolicystore.Record) error {
					called = true
					return nil
				})
			...
			t.Run("Put with IfMatch on a bucket without a policy fails", func(t *testing.T) {
				tenantID, bucketID := newBucket(t, "alice")
				_, err := s.Put(t.Context(), bucketpolicystore.Input{
					Bucket: bucketID, Tenant: tenantID, Policy: doc(allow([]string{"alice"}, "s3:GetObject")), IfMatch: ptr(`"nope"`),
				}, nil)
				require.ErrorIs(t, err, store.ErrPreconditionFailed)
				_, err = s.Get(t.Context(), bucketID)
				require.ErrorIs(t, err, store.ErrRecordNotFound)
			...
			t.Run("ListByPrincipal isolates by tenant", func(t *testing.T) {
				...
				_, err := s.Put(t.Context(), bucketpolicystore.Input{Bucket: bucketID, Tenant: tenantID, Policy: doc(allow([]string{"*"}, "s3:GetObject"))}, nil)
				require.NoError(t, err)
				_, err = s.Put(t.Context(), bucketpolicystore.Input{Bucket: otherBucket, Tenant: otherTenant, Policy: doc(allow([]string{"alice"}, "s3:GetObject"))}, nil)
				require.NoError(t, err)
			...
			t.Run("Put rewrites the index so a principal dropped from the document is no longer listed", func(t *testing.T) {
				...
				_, err = s.Put(t.Context(), bucketpolicystore.Input{
					Bucket: bucketID, Tenant: tenantID, Policy: doc(allow([]string{"bob"}, "s3:GetObject")), IfMatch: ptr(etag),
				}, nil)
				require.NoError(t, err)
...
func TestPolicyStorePostgres(t *testing.T) {
	...
	t.Run("index rows: one per named principal and a NULL row for the wildcard", func(t *testing.T) {
		...
		_, err := s.Put(t.Context(), bucketpolicystore.Input{
			...
			Policy: doc(allow([]string{"alice", "bob"}, "s3:GetObject"), deny([]string{"alice", "*"}, "s3:PutObject")),
		}, nil)
		require.NoError(t, err)
	...
	t.Run("Put rejects a principal the tenant does not have", func(t *testing.T) {
		...
		_, err := s.Put(t.Context(), bucketpolicystore.Input{
			Bucket: bucketID, Tenant: tenantID, Policy: doc(allow([]string{"alice", "mallory"}, "s3:GetObject")),
		}, nil)
		require.ErrorIs(t, err, store.ErrInvalidArgument)
		_, err = s.Get(t.Context(), bucketID)
	...
	t.Run("Put rejects a bucket that does not exist", func(t *testing.T) {
		tenantID := fx.tenant(t)
		_, err := s.Put(t.Context(), bucketpolicystore.Input{
			Bucket: testutil.RandomDID(t), Tenant: tenantID, Policy: doc(allow([]string{"*"}, "s3:GetObject")),
		}, nil)
		require.ErrorIs(t, err, store.ErrInvalidArgument)
	})
	...
	t.Run("a share-locked Get during a failed Put sees the old policy once it rolls back", func(t *testing.T) {
		...
		go func() {
			_, err := s.Put(context.Background(), bucketpolicystore.Input{
				Bucket: bucketID, Tenant: tenantID, Policy: doc(allow([]string{"alice"}, "s3:PutObject")), IfMatch: ptr(etag1),
			}, func(context.Context, *bucketpolicystore.Record) error {
				close(inCallback)
				<-release
