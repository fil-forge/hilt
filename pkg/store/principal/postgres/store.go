...
// the same principal, and a longer wait returns [store.ErrLockTimeout].
...
func (s *Store) Get(ctx context.Context, tenant did.DID, externalID string, locks ...store.LockMode) (rec principal.Record, err error) {
	defer func() { err = pglock.MapError(err) }()
	const query = `
...
// calls over overlapping principals from deadlocking; the wait for a row a
// removal holds is bounded at [store.LockTimeout] and a longer one returns
// [store.ErrLockTimeout].
func (s *Store) Lock(ctx context.Context, tenant did.DID, externalIDs []string, fn func(ctx context.Context) error) (err error) {
