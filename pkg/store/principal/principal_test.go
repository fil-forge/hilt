...
func TestPrincipalStorePostgresLocking(t *testing.T) {
	...
	t.Run("a share-locked Get waits for Lock's callback", func(t *testing.T) {
		...
		go func() {
			lockDone <- s.Lock(context.Background(), tenantID, []string{"held"}, func(ctx context.Context) error {
				close(entered)
				<-release
				return nil
			})
		...
		select {
		case <-getDone:
			t.Fatal("share-locked Get returned while Lock held the row")
		case <-time.After(grace):
		}
		...
		case err := <-getDone:
			require.NoError(t, err)
		case <-time.After(10 * time.Second):
			t.Fatal("share-locked Get did not return after Lock committed")
		}
	})
