...
// lockNamespace is the first key of the bucket-keyed transaction advisory
// lock every write takes and every share-locked read waits on. A row lock
// alone cannot cover a create: there is no row to lock until the INSERT
// commits, and an uncommitted INSERT is invisible to a FOR SHARE reader, so
// without this a reader arriving between the publish and the commit would
// answer "no policy" from the old state.
...
func (s *Store) Get(ctx context.Context, bucket did.DID, opts ...store.ReadOption) (policystore.Record, error) {
	...
	if err != nil {
		return policystore.Record{}, fmt.Errorf("beginning transaction: %w", err)
	}
	...
	if _, err := tx.Exec(ctx, `SET LOCAL lock_timeout = '`+shareLockTimeout+`'`); err != nil {
		return policystore.Record{}, fmt.Errorf("setting lock timeout: %w", err)
	}
	if err := advisoryLock(ctx, tx, bucket, true); err != nil {
		return policystore.Record{}, err
	}
	rec, err := scanRecord(tx.QueryRow(ctx, query+` FOR SHARE`, bucket.String()))
	return getResult(rec, err)
}

func getResult(rec policystore.Record, err error) (policystore.Record, error) {
	if errors.Is(err, pgx.ErrNoRows) {
		return policystore.Record{}, store.ErrRecordNotFound
	}
	if err != nil {
		return policystore.Record{}, fmt.Errorf("getting policy: %w", err)
	}
	return rec, nil
}

// advisoryLock takes the bucket's transaction-scoped advisory lock: exclusive
// for a writer, shared for a reader. It is released when tx ends.
...
func (s *Store) Put(ctx context.Context, in policystore.Input, beforeCommit func(ctx context.Context, old *policystore.Record) error) (string, error) {
	...
	canonical := policy.Canonical(in.Document)
	etag := policy.ETag(in.Document)
	if old == nil {
		_, err = tx.Exec(ctx, `
	...
	if err := writeIndex(ctx, tx, in.Bucket, in.Tenant, in.Document); err != nil {
		return "", err
	}
...
func lockRow(ctx context.Context, tx pgx.Tx, bucket did.DID) (*policystore.Record, error) {
...
func writeIndex(ctx context.Context, tx pgx.Tx, bucket, tenant did.DID, doc policy.Document) error {
...
func scanRecord(row pgx.Row) (policystore.Record, error) {
	var (
		bucketStr string
		document  []byte
		etag      string
	...
	if err != nil {
		return policystore.Record{}, fmt.Errorf("parsing bucket DID: %w", err)
	}
	var doc policy.Document
	if err := json.Unmarshal(document, &doc); err != nil {
		return policystore.Record{}, fmt.Errorf("decoding policy document: %w", err)
	}
	return policystore.Record{Bucket: bucket, Document: doc, ETag: etag, UpdatedAt: updatedAt}, nil
}
