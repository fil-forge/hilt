...
func TestAccessKeyStorePostgresIntegrity(t *testing.T) {
	...
	t.Run("the check constraint ties NULL lists to the principal", func(t *testing.T) {
		// Input.Validate refuses these before the write; the constraint is the
		// schema's own guarantee, so drive it directly.
		tenantID := testutil.RandomDID(t)
		seed.tenant(t, tenantID)
		seed.principal(t, tenantID, "user-1")
		for name, row := range map[string]string{
			"principal-bound with permissions":   `'{}', '{s3:GetObject}', 'user-1'`,
			"principal-bound with empty lists":   `'{}', '{}', 'user-1'`,
			"principal-bound with a bucket list": `'{did:key:b}', NULL, 'user-1'`,
			"service key with NULL permissions":  `NULL, NULL, NULL`,
		} {
			_, err := pool.Exec(t.Context(), `
				INSERT INTO access_key (id, tenant_id, name, buckets, permissions, principal_id)
				VALUES ($1, $2, 'k', `+row+`)
			`, testutil.RandomDID(t).String(), tenantID.String())
			require.ErrorContains(t, err, "access_key_principal_unscoped", name)
		}
	})
}
