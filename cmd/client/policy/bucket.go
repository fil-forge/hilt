...
func readDocument(cmd *cobra.Command, path string) (api.BucketPolicy, error) {
	...
	// The same strict decode the server applies, so a typo in a field name is
	// reported here rather than as a 422 from Hilt.
	doc, err := bucketpolicy.Decode(data)
	if err != nil {
...
func init() {
	for _, cmd := range []*cobra.Command{replaceCmd, deleteCmd} {
		cmd.Flags().StringVar(&ifMatch, "if-match", "", "the ETag the bucket's current policy must carry, which `policy get` reports")
